  ...
  const load = useCallback(async () => {
    ...
    if (off) {
      ...
      if (res.success && res.data) {
        ...
        const cached = await getCachedTripDetail(tripId);
        if (cached) { setTrip(cached); setOffline(true); loadedTrip = cached; }
      }
    }
    setLoading(false);
  ...
  return (
    ...
    <ScrollView style={s.root} contentContainerStyle={s.content}>
      ...
      <View style={[common.card, { marginBottom: 12 }]}>
        ...
        <TextInput
          style={[s.notesInput, { color: colors.text1, backgroundColor: colors.bgMuted, borderColor: colors.border }]}
        ...
        />
        <TouchableOpacity
          style={[s.notesSaveBtn, { backgroundColor: colors.orange }]}
          onPress={async () => {
            setSavingNotes(true);
